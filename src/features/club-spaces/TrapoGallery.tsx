...
export function TrapoGallery({ photos }: { photos: MediaAsset[] }): React.ReactElement {
  const [openIndex, setOpenIndex] = useState<number | null>(null);

  if (photos.length === 0) {
    return <p className="text-sm text-club-ink-muted">Aún no hay trapos cargados para este club.</p>;
  }
